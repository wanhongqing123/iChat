...
**Tech Stack:** Swift 5.9+ · iOS 16+ · Xcode 15+ · SwiftUI · NavigationStack · async/await · XCTest · ViewInspector（仅可选；本计划用 XCUITest 覆盖 UI 行为）
...
## Task 1: Scaffold Xcode 工程

**Files:**
- Create via Xcode UI: `UI/iOS/iChatLogin.xcodeproj`、初始 `iChatLoginApp.swift`、`ContentView.swift`、`Info.plist`、单元测试 target、UI 测试 target

- [ ] **Step 1: 用 Xcode 创建工程**

打开 Xcode：File → New → Project → iOS → App，参数：

| 字段 | 值 |
|---|---|
| Product Name | `iChatLogin` |
| Team | （任意，可留 None） |
| Organization Identifier | `com.ichat` |
| Bundle Identifier | 自动 = `com.ichat.iChatLogin` |
| Interface | SwiftUI |
| Language | Swift |
| Storage | None |
| Include Tests | ✅（同时勾选 Unit + UI Tests） |

保存位置选择：仓库根目录 / `UI/iOS/`（即 Xcode 在 `UI/iOS/` 下创建 `iChatLogin.xcodeproj` 和源码）。

- [ ] **Step 2: 调整 Deployment Target**

Project → iChatLogin → General → Deployment Info：iOS Deployment Target = `16.0`。

- [ ] **Step 3: 锁定竖屏**

Project → iChatLogin → General → Deployment Info → Device Orientation：仅勾选 `Portrait`。
...
```bash
cd UI/iOS && xcodebuild -scheme iChatLogin -destination 'platform=iOS Simulator,name=iPhone 15' build | tail -20
```

Expected：`** BUILD SUCCEEDED **`。

或：在 Xcode 中按 ⌘R，模拟器启动，看到默认 "Hello, world!" 界面。

- [ ] **Step 5: 提交**

```bash
git add UI/iOS
git commit -m "feat(ios): scaffold SwiftUI Xcode project (iOS 16+, portrait)"
```
...
```bash
xcodebuild -scheme iChatLogin -destination 'platform=iOS Simulator,name=iPhone 15' build | tail -10
```
...
final class PhoneUtilsTests: XCTestCase {
    ...
    func test_format_appliesGroupedSpacing() {
        ...
        XCTAssertEqual(PhoneUtils.format("13800013"),      "138 0013")
        XCTAssertEqual(PhoneUtils.format("138001380"),     "138 0013 8")
...
```bash
xcodebuild test -scheme iChatLogin -destination 'platform=iOS Simulator,name=iPhone 15' -only-testing:iChatLoginTests/PhoneUtilsTests | tail -20
```
...
```bash
xcodebuild -scheme iChatLogin -destination 'platform=iOS Simulator,name=iPhone 15' build | tail -5
```
...
```bash
xcodebuild test -scheme iChatLogin -destination 'platform=iOS Simulator,name=iPhone 15' -only-testing:iChatLoginUITests | tail -10
```
